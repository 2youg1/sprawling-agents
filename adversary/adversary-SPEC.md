...
**这条承诺反过来约束模型的记账**：被拒的命令同样花掉了它那把钥匙，故 `worldMinted`
必须在负向动作之后照样前进。`quickcheck-dynamic` 的 `failureNextState` 默认是恒等，
照默认走则下一条命令带着城已经答过的钥匙，读回来的是**上一条命令**的那份拒绝——
看上去像门读帧落后一条，实则是钥匙重了。只有查询不花钥匙，故 `Look` 是唯一的例外。
