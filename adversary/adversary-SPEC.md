...
| `WIRE_V = 16`，24 个 Command、20 个 Query（card-11.7 使 `Dispatch` 不再携 `budget`，card-5.4 增 `Command::PutDocument` 与 `Query::Governance`，card-2.6 增 `Query::Hunks`，card-6.5 增 `Query::Rounds`／`Evidence`／`CostOf`） | `crates/channels/src/wire.rs`、`wire/query.rs`、`command/kind.rs` |
