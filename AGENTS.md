...
- Working records live in one gitignored directory named in `.gitignore`, and `xtask release` refuses a published document that sends a reader into it — which is why this line does not name it either. `.gitignore` also refuses the file names such records arrive under, so the common mistake cannot be committed rather than merely being forbidden.
...
- Name a file by its repo-relative path: `crates/kernel/src/secret.rs`. An absolute path that starts at somebody's home directory names the machine as well as the file, and `xtask release` refuses one — including in a sentence explaining this rule, which is why this one describes the shape instead of spelling it.
