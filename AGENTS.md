...
| Keep the client's lockfile in step with its manifest, its runtime dependencies exactly `solid-js` and `effect`, and every licence in its installed tree on the list `deny.toml` permits. | `xtask npm` |
