...
import type { FileChange, GitOid, RoundsAnswer, RunId, Turn } from "../wire";
...
function Hunks(props: { readonly base: GitOid; readonly head: GitOid; readonly path: string }) {
  const ui = useUi();
  const say = useSay();
...
function Changes(props: { readonly base: GitOid; readonly head: GitOid | null }) {
  const ui = useUi();
  const say = useSay();
