...
| **WriteDomain** | The set of prefixes a resident may write, and what it may write inside them: `Everything`, or `Documents` — Markdown files only, and never a plan file. The decision primitive is `Address::is_within`; a building declares the second half with one `write` key in its `RULES.toml`. |
| **Documents** | The narrower of the two write-domain kinds: inside its prefixes a resident may write Markdown files and nothing else, and never a plan file. A building declares it with one `write` key in its `RULES.toml`; the wider kind is `Everything`. It is what a resident who plans is given, so a planner cannot reach into what a builder produces. |
...
| **connection kind** | How one registered endpoint is talked to, settled once when it is attached and never derived again: an OpenAI-compatible chat face, the responses face, the Anthropic native face, or a **harness family** — one of the four first-party clients whose subscription this city signs in to directly (`Codex`, `ClaudeCode`, `GrokBuild`, `KimiCli`). It is written into `endpoint_attached` and read back by `Query::Config`, so no call path re-derives it. A family is neither a URL nor a dialect: the city installs none of those clients and carries none of their code, and follows only their published facts ([`third-party.md`](third-party.md) §1). |
...
| **transcription** | The line of text a **recording** of speech became, produced by an optional endpoint speaking the OpenAI audio wire. Never called a transcript — that word already names what one run saw. Optional means the city works without one attached and says so in a refusal a person can act on, rather than answering nothing. |
...
| **desktop connector** | `sprawling-desktop`, an out-of-tree MCP server a building may attach: windows, snapshot, act, screenshot, record, clipboard, in the browser tool's vocabulary, inside an allowlist the building wrote in `DESKTOP.toml`. Two doors stand in front of it, in this order: `desktop = true` in the building's `RULES.toml` decides whether it may be attached at all, and the allowlist decides which windows on this machine it may then touch. |
| **doctor** | `sprawling doctor`: what this machine has against what a city needs, in two tiers — enough to use, enough to develop — and, with `--install`, one consented installation at a time. It is the one authority on what this machine has: the exec tool and the browser engine ask it rather than the machine. With a city named it judges each building's capability bits (`browser`, `desktop`, `sandbox.shell`) against this machine; with `--explain <code>` it connects a refusal code to the items behind it. What it installs lands in `~/.sprawling/components/`, at machine level and never inside a city. |
...
| **managed span** | A number in a document that a gate owns, wrapped in a pair of HTML comments that name the fact — the opening one spells `xtask:begin` and the fact's key, the closing one spells `xtask:end`. `cargo xtask docnum` recounts every such fact from the code that decides it and refuses a span whose text disagrees; `--write` corrects it. It is how a document quotes a figure without becoming a second home for it — the defect it closes is prose that stated `WIRE_V` as 15 while the wire had reached 31. |
| **not-proved marker** | The line `// not-proved: <reason>` above a kani harness that is written but is not proved today, stating the reason where the next reader is already standing. `cargo xtask proof` reads it to skip that harness and print why, so the source is the only roster and no second list can go stale. Deleting the line is how a harness comes back; today no harness carries one. |
