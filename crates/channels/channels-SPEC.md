...
### 8-27 说出来的那句话：`/transcribe` 与 `ModelTag::Transcribe`（card-6.10；WIRE_V 20→21）
...
### 8-26 `ConfigureBuilding` 长出 `desktop`：一栋楼的桌面白名单走同一条帧（card-7.3；WIRE_V 19→20）
...
### 8-25 `Query::Doctor`：这台机器有什么（card-9.2；WIRE_V 18→19）
